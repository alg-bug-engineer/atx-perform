...
| 2 | 分析成因 | 隐去渠化，上/下游流量溯源（≤3 跳，份额）+ 供需与下游约束 |
...
| 经十东进口车道 | 实地覆盖库内渠化 | **7 直行 + 1 右转 + 2 左转**（主路+辅路合计 10） |
...
- 动作：隐去双路口渠化；以经十路–奥体西为目标路口，对北进口直行/右转/左转做 **上游 + 下游** 溯源（最多 3 跳），水流汇入/去向可视化；路段旁标注需求与供给。
...
- **信号协调**：点明两口绿灯时间不一致——经十北直仍为红灯时解放东继续放行，车流压入短路段，溢流风险加重；共同周期一致，具备协调条件。优化秒数留幕 3。
- 数据：`1-2-cause-analysis.json` 的 `upstream_traces` / `downstream_traces` / `signal_coordination_evidence`。
