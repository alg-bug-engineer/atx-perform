...
| 2 分析成因 | `features/scenes/scene2-cause.js`（trace/supply/ew_clear/arterial/signal/overflow 节拍 + `inflowTraceLayer` / `jingshiEwFlowLayer` / `scene2MapAnnot`），溯源只用 `flow_share_ratio` | `data/1-2-flow-trace.json`（3D 运行时按 `/data/` 取，见 `serveRepoData`）、`data/1-2-cause-analysis.json` |
